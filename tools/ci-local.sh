...
python3 scripts/sac_pack.py services/order-service.md --bundle sample-knowledge --json | python3 -c "import sys,json; d=json.load(sys.stdin); assert d['node_count']>=3, d; assert d.get('reverse_index') in ('rg','scan'), d"
