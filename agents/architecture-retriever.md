...
This is a **query-time retriever**, not a reverse-engineering walker. Do not confuse this role with `codebase-walker`, `iac-reverse-engineer`, `network-iam-topology`, `cicd-reverse-engineer`, `identity-auth-discoverer`, `wiki-ticket-ingester`, or `graph-builder`. Do **not** spawn this agent during reverse-engineering plan / fan-out — walkers populate the graph; you only read it afterwards.
